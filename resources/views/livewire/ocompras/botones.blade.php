    ...
    @can('gestionar', $row)
        @if($row->estatus === 'aprobado')
            <button wire:click="cambiarEstatus({{ $row->id }}, 'ordenado')" 
                class="bot botAzul">Ordenar
            </button>
        @endif
        @if($row->estatus === 'ordenado')
            <button wire:click="cerrarRecepcion({{ $row->id }})" 
                onclick="confirm('¿Afectar inventario y cerrar orden?') || 
                event.stopImmediatePropagation()" class="bot botVerde">Recibir
            </button>
        @endif
        @can('eliminar', $row)
            @if(!in_array($row->estatus, ['recibido', 'cancelado']))
                <button wire:click="cambiarEstatus({{ $row->id }}, 'cancelado')" 
                    class="bot botRojo" onclick="confirm('¿Cancelar esta orden?') || 
        ...
        @can('eliminar', $row)
            @if($row->estatus === 'edicion')
                <button wire:click="destroy({{ $row->id }})" onclick="confirm('¿Eliminar registro?') 
                    || event.stopImmediatePropagation()" class="bot botRojo"><i class="bi bi-trash"></i>
                </button>
            @endif
        @endcan     
    @endcan

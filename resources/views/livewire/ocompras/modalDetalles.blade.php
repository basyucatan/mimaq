    ...
    <table class="table tabBase ch">
        ...
        <tbody>
            ...
            <tr class="align-middle">
                <td><input type="number" step="1" class="inpSolo" wire:model.live="detalles.{{$idx}}.cantidad"></td>
                <td>
                    <div class="d-flex gap-2">
                        @if($det['colorRgba'])
                        <span class="cuadroColor" style="background-color: {{ $det['colorRgba'] }}; width: 10px; height: 10px; border-radius: 2px;"></span>
                        @endif
                ...
                <td><input type="number" wire:model.live="detalles.{{$idx}}.costoU" class="inpSolo text-end p-1"></td>
                <td class="text-end fw-bold text-dark">${{ number_format((float)$det['cantidad'] * (float)$det['costoU'], 2) }}</td>

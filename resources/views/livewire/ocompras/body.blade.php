    <div class="cardPrin-body">
        ...
        <div class="table-responsive d-none d-md-block">         
            <table class="table tabBase">
                ...
                <tbody>
                    ...
                    <tr>
                        @php
                            $clase = match($row->estatus){
                                'edicion' => 'bg-warning text-dark',
                                'ordenado' => 'bg-warning text-dark',
                                'cancelado' => 'bg-black',
                                'aprobado' => 'bg-success',
                                'recibido' => 'bg-primary',
        ...
        <div class="d-md-none">        
            @forelse($ocompras as $row)
                @php
                    $claseBorde = match($row->estatus){
                        'edicion', 'ordenado' => 'border-warning',
                        'cancelado' => 'border-dark',
                        'aprobado' => 'border-success',
                        'recibido' => 'border-primary',
                        default => 'border-secondary'
                    };
                @endphp            
                <div class="card border mb-2 shadow-sm border-start border-4 {{ $claseBorde }}">
                    <div class="card-body p-2">
                        ...
                        <div>
                            🚧<span class="fw-bold">{{ $row->Obra->obra }} ({{ implode(' ', array_slice(explode(' ', $row->Obra->Cliente->empresa), 0, 2)) }})</span>
                        </div>
        ...
        @include('livewire.ocompras.modals.principal')
        @include('livewire.ocompras.modals.destino')     
        @include('livewire.ocompras.modals.soloDetalles')           
    </div>
...
<div class="col-md-6">
    ...
    <input wire:model.live="cliente" list="listaClientes" type="text" class="inpBase" onfocus="this.select()" autocomplete="off">
    <datalist id="listaClientes">
        @foreach($clientes as $id => $nombre)
            <option value="{{ $nombre }}">
        @endforeach
    </datalist>
    @error('IdCliente') <span class="error text-danger">Seleccione un cliente válido</span> @enderror
</div>                            
                                <div class="col-md-6">
                            ...
                            @elseif($tipoModal == 'Folio')
                                ...
                                <div class="col-md-2">
                                    ...
                                    <input wire:model="cantidad" wire:change="generarDef"type="number" class="inpBase">
                                ...
                                <div class="col-md-3">
                                    ...
                                    <div class="btn-group w-100" role="group">
                                        @foreach($kts as $k)
                                            <input type="radio" class="btn-check" wire:model="kt" wire:change="generarDef" 
                                                value="{{ $k }}" id="kt{{ $k }}" name="radioKt" autocomplete="off">
                                            <label class="btn btn-outline-secondary btn-sm pt-1" for="kt{{ $k }}" style="font-size: 11px;">
                                                {{ $k }}
                                            </label>
                                        @endforeach
                                    </div>
                                ...
                                <div class="col-md-2">
                                    ...
                                    <div class="btn-group w-100" role="group">
                                        @foreach($colors as $c)
                                            <input type="radio" class="btn-check" wire:model="color" wire:change="generarDef" 
                                                value="{{ $c }}" id="col{{ $c }}" name="radioColor" autocomplete="off">
                                            <label class="btn btn-outline-secondary btn-sm pt-1" for="col{{ $c }}" style="font-size: 11px;">
                                                {{ $c }}
                                            </label>
                                        @endforeach
                                    </div>
                                ...
                                <div class="col-md-7">
                                    ...
                                    <div class="inpBase" style="background: #f8f9fa; min-height: 31px; height: auto; padding: 5px;">
                                        @if(isset($adicionales['composicion']) && is_array($adicionales['composicion']))
                                            @foreach($adicionales['composicion'] as $idMat => $detalle)
                                                <span class="badge bg-warning text-black mb-1" title="ID Material: {{ $idMat }}">

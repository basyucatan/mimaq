...
<div class="container-fluid p-0">
    <div class="row g-0 justify-content-center">
        <div class="col-12">
            <div class="cardPrin">
                <div class="cardPrin-header d-flex flex-column flex-md-row justify-content-between align-items-stretch align-items-md-center gap-2 p-2">
                    <span class="fs-5 fw-bold text-nowrap align-self-center">Administración de Bandejas</span>
                    <div class="d-flex flex-column flex-md-row align-items-stretch align-items-md-center gap-2 flex-grow-1 justify-content-end">
                        <div class="d-flex flex-column flex-sm-row align-items-stretch align-items-md-center gap-2">
                            <div class="position-relative">
                                <input wire:model.lazy="selected_id" wire:change="escanear" class="inpSolo" wire:keydown.escape="$set('selected_id','')" onfocus="this.select()" placeholder="Escanear...">
                                @if($selected_id)
                                    <span wire:click="$set('selected_id','')" class="bot botNegro botChico" style="position: absolute; right: 6px; top: 50%; transform: translateY(-50%); cursor: pointer;">X</span>
                                @endif
                            </div>
                            <div class="position-relative">
                            ...
                            <div class="position-relative">
                                ...
                                @error('IdFactura')
                                    <span class="text-danger small position-absolute start-0 top-100">{{ $message }}</span>
                                @enderror
                            </div>
                        </div>
                        <div class="d-flex align-items-center justify-content-sm-end gap-2 mt-1 mt-md-0">
<button
    class="bot {{ $IdFactura ? 'botVerde' : 'botGris' }}"
    wire:click="exportar"
    title="Exportar Bandejas Terminadas"
    {{ $IdFactura ? '' : 'disabled' }}>
                        ...
                        <tbody>
                            @forelse($bandejas as $row)
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <span class="d-inline-block styleProceso" style="max-width: 140px;">
                                    ...
                                    <td class="text-center">
                                        <i
                                            class="bi {{ $row->enBoveda ? 'bi-shield-check text-success fs-5' : 'bi-shield-x text-muted fs-5' }}"></i>
                                    </td>
                                    <td class="text-center">
